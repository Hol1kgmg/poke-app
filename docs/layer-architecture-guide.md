...
- `useXxx.ts` — UI状態の管理、または widget 固有のデータ取得（features まで下げられない場合）
...
- `state/` — 複数レイヤーをまたぐグローバル状態（`currentUser`・`sidebarOpen` 等）
...
**features — entities の状態に依存、またはユーザーアクションを伴うデータ取得**
...
BFF リクエストを定義したい
  ...
  → No  ↓
  features に置ける？
  （entities / shared の依存で足りるか、またはユーザーアクションか）
