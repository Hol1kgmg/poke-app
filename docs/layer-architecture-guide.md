...
> `aggregates/` は FSD 標準に存在しないレイヤー。複数 composite エンティティをまたぐ型定義と UI の置き場所として、entities 間の cross-import 例外を最小化するために導入した。責務は後述の通り極めて限定的。
...
### 5. 外部非公開のファイルを持つスライスは index.ts で公開 API を定義する

スライス内に外部非公開のファイルが存在する場合、`index.ts` を置いて公開するものだけを列挙する。スライス外のコードは `index.ts` 経由でのみ import できる。内部ファイルへの直接 import は禁止。

- 外部非公開のファイルを持たないスライスは `index.ts` 不要
- `widgets/` は内部分割コンポーネントを持つ設計のため原則 `index.ts` 必須
- `shared/ui/` はコンポーネントごとの個別 import が自然な構造のため対象外

### 6. サーバー／クライアントの境界はレイヤーではなくコンポーネント単位で決定する
...
src/
  ...
  aggregates/                       ← 複数 composite エンティティをまたぐ型定義と UI（FSD 外の非標準層、責務は極めて限定的）
    pokemon-moves/
      model/
        types.ts                      PokemonWithMoves 等、複数 composite をまたぐ型定義
      ui/
        PokemonMovesCard.tsx
        PokemonMovesCard.module.css
...
- スライス内の `index.ts` からのみ参照する（スライス外からの内部ファイルへの直接 import は禁止）
- `index.ts` には外部に公開するものだけを列挙し、内部分割コンポーネントは含めない
...
### aggregates/（複数 composite エンティティをまたぐ型定義と UI）

**FSD 標準には存在しないレイヤー。** entities 間の cross-import 例外を最小化するために導入した。責務は以下の3つのみに限定する。

- 複数の composite エンティティをまたぐ**型定義**（`PokemonWithMoves` 等）
- その型を返す**データ取得**（useQuery）
- その型を表示する**再利用 UI コンポーネント**（`PokemonMovesCard` 等）

これ以外のものは aggregates に置かない。

```
✅ aggregates に置くもの
  pokemon-moves/model/types.ts   — Pokemon（composite）と Move（composite）をまたぐ型定義
  pokemon-moves/ui/              — PokemonWithMoves を表示する再利用 UI パーツ
  pokemon-moves/usePokemonMoves.ts — aggregate 型のデータ取得（useQuery）

❌ aggregates に置かないもの
  composite → atomic の cross-import   → entities 内で解決できる
  ユーザーアクション（mutation）          → features
  ページ固有の複合ブロック               → widgets
  aggregate 型と無関係な BFF リクエスト  → entities / features / widgets
```

> **なぜ widgets に置けないか：** widgets に置くと、他の widget がそれを import する際に同レイヤー間 import（禁止パターン #1）が発生する。aggregates は widgets より下位に位置するため、widgets から自由に import できる。
...
スライス名は名詞（kebab-case）。mutation・atoms は持たない。
...
「単一のドメインリソース」は **DB テーブル 1 枚**に対応し、型定義ではテーブル型（`Pokemon`）とカラムの Branded Type（`PokemonId`）がその最小単位となる。
...
entities は原則として同階層間の import を禁止するが、**composite → atomic の一方向に限り例外を認める**。

| 用語 | 定義 | 例 |
|---|---|---|
| atomic | 他の entity に依存しない最小単位 | `pokemon-type`（FK 先を持たない） |
| composite | atomic entity の型・UI を参照する | `pokemon`（`PokemonTypeId[]` を持つ） |

**許可する方向：**

| 方向 | 型定義 | UI |
|---|---|---|
| composite → atomic | ✅ `Pokemon` が `PokemonTypeId` を参照 | ✅ `PokemonCard` が `TypeBadge` を import |
| atomic → composite | ❌ | ❌ |
| atomic ↔ atomic | ❌ | ❌ |
...
型定義では1ファイル内でカラム型・テーブル型を共存できるが、UIではファイル分割が必要なためディレクトリ間の cross-import として現れる。どちらも同じ方向制限が適用される。
...
UIコンポーネントを作りたい
  ...
  ② 「動詞+名詞」で命名できる操作・機能か？
     Yes → features/
     No  ↓
  ③ 1つの composite エンティティ（またはその atomic 依存）に収まる再利用パーツ？
     Yes → entities/xxx/ui/
     No  ↓
